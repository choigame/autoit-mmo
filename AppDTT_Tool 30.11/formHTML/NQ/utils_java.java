...
public class UTILSNAMEBYTANUtils {
	...
	public void saveUTILSNAMEBYTAN(ActionRequest request, ActionResponse respone) 
												throws Throwable {
	...
	public void updateUTILSNAMEBYTAN(ActionRequest request, ActionResponse respone) 
		...
		HttpSession session = ActionUtils.getSession(request);
		ENTITYNAMEBYTAN obj = fillData2Form(request);session.setAttribute(GROUPCONSTANTBYTAN.OBJECT_DATA1,obj);
		session.setAttribute(GROUPCONSTANTBYTAN.OBJECT_DATA2,hoSoTTHCCong);
		request.setAttribute(GROUPCONSTANTBYTAN.ID_QUY_TRINH,ParamUtil.getString(request, "idQuyTrinh"));
