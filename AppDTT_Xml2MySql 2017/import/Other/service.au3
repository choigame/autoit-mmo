...
Func createColumnLong()
  if($serviceFilePath='') Then
   ...
   ElseIf (StringInStr($select,'giayToCmnd ***')) Then
	  $content1 = "<column name=" &'"'&'cmnd'&upperFirstLetter($str)&'" ' & "db-name=" &'"' &'cmnd'& $str&'" ' &"type="&'"'&"String"&'"/>' &@CRLF& _
	  "<column name=" &'"'&'noiCapCmnd'&upperFirstLetter($str)&'" ' & "db-name=" &'"' &'noiCapCmnd'& $str&'" ' &"type="&'"'&"long"&'"/>' &@CRLF& _
	  "<column name=" &'"'&'ngayCapCmnd'&upperFirstLetter($str)&'" ' & "db-name=" &'"' &'ngayCapCmnd'& $str&'" ' &"type="&'"'&"Date"&'"/>'
   EndIf
   ...
   ElseIf(StringInStr($select,'giayToCmnd ***')) Then
	  $content2 = 'cmnd'&$str & ' varchar(' & $maxLengthInput & '),'  &@CRLF& _
				  'noiCapCmnd' & $str & " decimal," &@CRLF& _
				  'ngayCapCmnd' & $str & " datetime,"
   EndIf
   ...
   If(StringInStr($select,'giayToCmnd ***')) Then
	  $content3 = "String cmnd"& upperFirstLetter($str) &@CRLF& _
				  "long noiCapCmnd"& upperFirstLetter($str) &@CRLF& _
				  "Date ngayCapCmnd"& upperFirstLetter($str)
   ...
   ElseIf $select == 'address *' Then
	  $arrRequiredValidate &= quote('tinh'& upperFirstLetter($str)) &","& quote('quan'&upperFirstLetter($str)) &","& quote('xa'&upperFirstLetter($str)) &","& quote('diaChi'&upperFirstLetter($str)) &","
	  $arrRequiredValidateTab &= quote('tinh'& upperFirstLetter($str&$suffix)) &","& quote('quan'&upperFirstLetter($str&$suffix)) &","& quote('xa'&upperFirstLetter($str&$suffix)) &","& quote('diaChi'&upperFirstLetter($str&$suffix)) &","
   ElseIf $select == 'address' Then
	  $arrRequiredValidate &= quote('tinh'& upperFirstLetter($str)) &","& quote('quan'&upperFirstLetter($str)) &","& quote('xa'&upperFirstLetter($str)) &","
	  $arrRequiredValidateTab &= quote('tinh'& upperFirstLetter($str&$suffix)) &","& quote('quan'&upperFirstLetter($str&$suffix)) &","& quote('xa'&upperFirstLetter($str&$suffix)) &","
   ElseIf $select == 'giayToCmnd ***' Then
	  $arrRequiredValidate &= quote('cmnd'& upperFirstLetter($str)) &","& quote('noiCapCmnd'&upperFirstLetter($str)) & "," & quote('ngayCapCmnd'&upperFirstLetter($str)) &","
	  $arrRequiredValidateTab &= quote('cmnd'& upperFirstLetter($str&$suffix)) &","& quote('noiCapCmnd'&upperFirstLetter($str&$suffix)) & "," & quote('ngayCapCmnd'&upperFirstLetter($str&$suffix)) &","
   EndIf

   ;arrDate.txt
   $arrDateValidate &= quote('ngayCapCmnd'&upperFirstLetter($str)) &","
   $arrDateValidateTab &= quote('ngayCapCmnd'&upperFirstLetter($str&$suffix)) &","
...
Func fillData2FormLong($select,$str,$suffix)
   ...
   If StringInStr($select, 'address') Then
	 ...
	 ElseIf StringInStr($select, 'giayToCmnd ***') Then
		$a = 'String cmnd'&upperFirstLetter($str&$suffix)&' = ParamUtil.getString(request,"cmnd'&upperFirstLetter($str&$suffix)&'").trim();'&@CRLF& _
		 'hoSo.setCmnd'&upperFirstLetter($str&$suffix)&'(cmnd'&upperFirstLetter($str&$suffix)&');'&@CRLF& _
		 'String ngayCapCmnd'&upperFirstLetter($str&$suffix)&' = ParamUtil.getString(request,"ngayCapCmnd'&upperFirstLetter($str&$suffix)&'");'&@CRLF& _
		 'hoSo.setNgayCapCmnd'&upperFirstLetter($str&$suffix)&'(ActionUtils.parseStringToDate(ngayCapCmnd'&upperFirstLetter($str&$suffix)&'));'&@CRLF& _
		 'long noiCapCmnd'&upperFirstLetter($str&$suffix)&' = ParamUtil.getLong(request,"noiCapCmnd'&upperFirstLetter($str&$suffix)&'");'&@CRLF& _
		 'hoSo.setNoiCapCmnd'&upperFirstLetter($str&$suffix)&'(noiCapCmnd'&upperFirstLetter($str&$suffix)&');'
...
Func createReviewLong($select,$str)
   ...
   ElseIf $select == "giayToCmnd ***"  Then
	  $hand = _FileOpenTemplate($path, $reviewLongGiayToCmnd)
   EndIf
...
Func createValidateLong($select,$str,$suffix)
   ...
   Local $path = @ScriptDir & $jspFolderTemplate & 'create service\validate long\'
   Local $hand
   Local $value
   ...
   ElseIf StringInStr($select , 'giayToCmnd ***') Then
	  $hand = FileOpen($path & $validateLongGiayToCmnd)

	  If ($hand == -1) Then
		 MsgBox($MB_ICONERROR+262144+8192, $ErrorDiaglogTitle, $validateLongGiayToCmnd & " Not found!")
	  EndIf
   ...
   If $select == 'address' Then
	  $value = StringRegExpReplace($value, "(?s)<diaChi>(.*?)<diaChi>", "")
   ElseIf $select == 'address *' Then
	  $value = StringReplace($value,'<diaChi>','')
   EndIf

   $value = StringReplace($value,"TYPEBYTAN",$select)

   If StringInStr($select , 'address') Or StringInStr($select , 'giayToCmnd ***') Then
	  $value = StringReplace($value,"NAMEBYTAN", upperFirstLetter($str&$suffix))
   Else
	  $value = StringReplace($value,"NAMEBYTAN", $str&$suffix)
   EndIf
...
Func registryFormLong($select,$str,$suffix,$isMsg)
   ...
   ElseIf (StringInStr($select,'giayToCmnd ***')) Then
	  $value = _registryFormLongType2($str, $suffix,0,$isMsg,$registryLongGiayToCmnd)
   ElseIf (StringInStr($select,'address *')) Then
	  $value = _registryFormLongType2($str, $suffix, 1, $isMsg,$registryLongAdress)
   ElseIf (StringInStr($select,'address')) Then
	  $value = _registryFormLongType2($str, $suffix, 0, $isMsg,$registryLongAdress)
   EndIf
...
Func _registryFormLongType2($str,$suffix,$isRequired,$isMsg,$fileName)
   ...
   Local $value = FileRead($hand)
   $value = StringReplace($value,"NAMEBYTAN",$str&$suffix)
   $value = StringReplace($value,"UPPERFIRSTLETTER", $str)
   $value = StringReplace($value,"MAXLENGTHBYTAN",$maxLengthInput)

   If ($isRequired == 1) then Return $value

   Return StringReplace($value,'<label class="egov-label-red upper">*</label>','')
